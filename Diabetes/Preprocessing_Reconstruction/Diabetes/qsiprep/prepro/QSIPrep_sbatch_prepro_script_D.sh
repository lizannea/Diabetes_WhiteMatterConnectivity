...
#-----------------------------------------
#
# A batch script to pre-process the Deakin dataset
#
################################################################################
#
# Usage             : QSIPrep_sbatch_prepro_script_D.sh
...
apptainer run --cleanenv `#  host environment variables to pass into the container`\
--nv `# setup the containers environment to use an NVIDIA GPU and the basic CUDA libraries`\
-B /fred/oz251/Diabetes:/data `#b data directory`\
...
--verbose \
-w /data/derivatives/LA/qsiprep/work >& /fred/oz251/Diabetes/derivatives/LA/qsiprep/log/qsiprep_${subj}_$(date -Iseconds).log
