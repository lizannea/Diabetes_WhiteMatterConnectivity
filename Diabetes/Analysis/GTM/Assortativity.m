...
resultsDir = '/Users/lizanne/Connectome/Diabetes_study/Results';
...
% Loop through variables in workspace and calculate assortativity for each subject
for i = 1:length(varList)
    % Check if variable starts with 'sub_*'
    if startsWith(varList{i}, 'sub_')
        % Calculate assortativity
        r = assortativity_wei(eval(varList{i}), 0);
        % Extract subject ID from variable name
        subID = extractAfter(varList{i}, 'sub_');
        % Add results to table
        resultsTable(end+1,:) = {subID, r};
    end
end

% Set column names
resultsTable.Properties.VariableNames = {'SubjectID', 'r'};
...
writetable(resultsTable, fullfile(resultsDir, 'assortativity_results.csv'), 'WriteRowNames', true);
